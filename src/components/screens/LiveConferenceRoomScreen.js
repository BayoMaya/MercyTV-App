...
export default class LiveConferenceRoomScreen extends Component {
  ...
  async componentWillMount() {
    await OpenTok.connect(opentoksessionId, opentokcodetoken);
  ...
  async componentDidMount(){
      ...
      await OpenTok.sendSignal(opentoksessionId, 'message', 'a');
      alert(isSent); 
      this.initTwilio();
  }
    ...
    return (
      <View style={styles.container}>
        <View style={{ flex: 2, alignItems: 'stretch', flexDirection: 'column' }}>
          <Publisher sessionId={opentoksessionId} style={{ height: 100, alignSelf: 'stretch', backgroundColor: 'black' }} cameraDirection="front" video={ true } ref={ref => { this.ref = ref; }}
          ...
          <Button
          onPress={async () => {
            const isSent = await OpenTok.sendSignal('1_MX40NjM0NDA1Mn5-MTU2MDE2MzI1OTIzMX5VNDJqblpZTnA3SjFqVUZiTkpKbXR5MWZ-UH4', 'message', 'a');
            console.log(isSent);
          }}
          title="Send signal"
          />
          ...
          title="Switch camera"
          />
          <Subscriber sessionId={opentoksessionId} style={{ height: 200, alignSelf: 'stretch', backgroundColor: 'white' }} 
